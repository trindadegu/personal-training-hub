...
import type { Aluno, HistoricoEntry } from "./types";
import type { Nota } from "./api/notas";
import type { Pagamento } from "./api/pagamentos";
import { formatBRL, formatDateBR } from "./api/config";

export function exportProntuarioPDF(opts: {
  aluno: Aluno;
  ...
  doc.setFontSize(11);
  doc.text(`Atlântida · Gerado em ${new Date().toLocaleString("pt-BR")}`, margin, 52);
  doc.setTextColor(0);
  y = 90;
...
export function exportFinanceiroPDF(opts: {
  ...
  receitas: number;
  despesas: number;
  lancamentos: Array<{ data: string; tipo: string; categoria: string | null; descricao: string; valor: number }>;
}) {
  const doc = new jsPDF({ unit: "pt", format: "a4" });
  const W = doc.internal.pageSize.getWidth();
  const margin = 40;
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(11);
  doc.text(`Mês ${opts.mes} · Gerado em ${new Date().toLocaleString("pt-BR")}`, margin, 52);
  doc.setTextColor(0);
